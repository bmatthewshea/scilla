...
module ScillaSanityChecker
  ...
  let basic_sanity (cmod : cmodule) =
          ...
          (fun (seen2 : loc list list) (stmt : stmt_annot) ->
            ...
            | MatchStmt (_ident, branches) ->
               ...
               List.fold_left
                 ...
                 [] branches
            | _ -> seen2
          ) (seen : loc list list) (stmts : stmt_annot list)
      in
        walk [] stmts
    in
    ...
    let check_accepts (contr : contract) =
      let check_transition_accepts (transition : transition) =
        ...
        let dup_accept_warning (group : loc list) : unit =
          (warn1
             (Core.sprintf "transition %s had a potential code path with duplicate accept statements:\n"
        ...
        List.iter
          (fun group ->
            match List.length group with
            | 0 -> ()
            | 1 -> ()
            | _ -> dup_accept_warning group)
    ...
    let e = e @ check_accepts contr in
